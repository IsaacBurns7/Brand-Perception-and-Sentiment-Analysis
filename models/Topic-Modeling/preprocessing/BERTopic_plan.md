...
## Sample Commands For BERTopic Training Script

### 1) Build BERTopic-ready preprocessed corpus
python LDA_normalize_corpus.py --input ../data/rating.csv --text-col article --output ../data/rating_normalized_for_BERTopic.csv --topic-model-target bertopic --bertopic-min-words 3 --bertopic-drop-empty-text --max-doc-count 10000 --batch-size 100

### 2) Train BERTopic on preprocessed output
python bertopic_train.py --input ../data/rating_normalized_for_BERTopic.csv --text-col cleaned_text --output-dir ../data/bertopic_outputs --embedding-model sentence-transformers/all-MiniLM-L6-v2 --min-topic-size 15 --ngram-min 1 --ngram-max 2

### 3) Train BERTopic with probabilities and auto topic reduction
python bertopic_train.py --input ../data/rating_normalized_for_BERTopic.csv --text-col cleaned_text --output-dir ../data/bertopic_outputs_prob --calculate-probabilities --nr-topics auto --min-topic-size 10

### 4) Quick smoke train on a small subset
python bertopic_train.py --input ../data/rating_normalized_for_BERTopic.csv --text-col cleaned_text --output-dir ../data/bertopic_outputs_smoke --max-doc-count 500 --min-topic-size 8

Artifacts generated by bertopic_train.py:
- document_topics.csv
- topic_info.csv
- bertopic_model/
