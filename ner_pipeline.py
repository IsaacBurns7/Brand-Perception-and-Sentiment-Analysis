...
class NERPipeline:
    ...
    text_column     : which DataFrame column to run NER on (default: full_content,
                      auto-resolved as: article → full_content → content)
    ...
    def run_on_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Picks the best available text column in priority order:
          article       (scraped full text — richest, used in rating.csv / news.py)
          → full_content (Preprocessing.runner output)
